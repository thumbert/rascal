...
abstract class Matrix {
  ...
  factory Matrix(List x, int nrow, int ncol, {bool byRow: false}) {
    ...
    if (x[0] is int) {
      return new IntMatrix(x, nrow, ncol, byRow: byRow);
    } else if (x[0] is double) {
  ...
  factory Matrix.filled(num value, int nrow, int ncol) {
    if (value is int){
      return new IntMatrix.filled(value, nrow, ncol);
    } else {
      return new DoubleMatrix.filled(value, nrow, ncol);
    }
  }
...
//    nrow = size;
//    ncol = size;
//    data = new List.filled(size * size, 0);
//    this.diag = new List.filled(size, 1);
//  }
   ...
   */
  void setElement(int i, int j, value) {
    data[i][j] = value;
  }
   ...
   */
  void setRow(int i, List values) {
    for (int j = 0; j < ncol; j++) data[i][j] = values[j];
  }
   ...
   */
  void setColumn(int j, List values) {
    for (int i = 0; i < nrow; i++) data[i][j] = values[i];
  }
   ...
   */
  set diag(List values) {
    int s = min(nrow, ncol);
    for (int i = 0; i < s; i++) data[i][i] = values[i];
  }
  ...
  Matrix transpose() {
    Matrix res;
    if (this is IntMatrix)
      res = new IntMatrix.filled(0, ncol, nrow);
    else
      res = new DoubleMatrix.filled(0.0, ncol, nrow);
  ...
  _populateMatrix(List x, List<List> data, {bool byRow: false}) {
    if (byRow) {
      for (int i = 0; i < nrow; i++)
        for (int j = 0; j < ncol; j++)
          data[i][j] = x[j + i * ncol];
...
/**
 * An efficient implementation of a diagonal matrix.
 */

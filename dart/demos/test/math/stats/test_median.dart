...
import 'package:demos/math/stats/quantile.dart';
...
testQuantile() {
  ...
  test('R1 quantile calculation, sorted array', () {
    ...
    Quantile q = new Quantile(x, shuffle: false, quantileEstimationType: QuantileEstimationType.R1);
  ...
  test('R4 quantile calculation, sorted array', () {
    ...
    Quantile q = new Quantile(x, shuffle: false, quantileEstimationType: QuantileEstimationType.R4);
    List probs = [0, 0.25, 0.5, 0.75, 1];
    var res = probs.map((p) => q.value(p)).toList();
  ...
  test('R7 quantile calculation, sorted array', () {
    List x = new List.generate(8, (i) => i);
    Quantile q = new Quantile(x, shuffle: false, quantileEstimationType: QuantileEstimationType.R7);
    List probs = [0, 0.25, 0.5, 0.75, 1];
    var res = probs.map((p) => q.value(p)).toList();
